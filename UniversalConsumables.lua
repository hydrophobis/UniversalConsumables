...
-- JokerDisplay passes joker cards to poker hand evaluation
...
get_X_same = function(num, hand, or_more)
  ...
  if #clean < num then return {} end
  return UC_orig_get_X_same(num, clean, or_more)
end

-- get_poker_hand_info can return nil for scoring_hand when non-playing
-- cards are in the hand (jokers/consumables). This crashes any caller
-- that does pairs(scoring_hand). Permanently wrap it to guarantee the
-- 4th return is never nil.
...
function UC.hand_selected()
  return G.hand and G.hand.highlighted and #G.hand.highlighted > 0 and not UC.non_hand_highlighted()
end
  ...
  SMODS.Consumable:take_ownership(key, {
    ...
    use = function(self, card, area, copier)
      ...
      G.E_MANAGER:add_event(Event({ trigger="after", delay=0.1,
        func=function()
          if target then
            target:set_seal(seal, true, true)
            card_eval_status_text(target,"extra",nil,nil,nil,{message=localize(msg_key),colour=G.C[colour_key]})
          end
          return true
        end
      }))
...
make_enhance2("hierophant", "m_bonus", "k_bonus_ex", "CHIPS")
...
SMODS.Consumable:take_ownership("death", {
  can_use = function(self, card)
    ...
    local hand_count = 0
    for _,c in ipairs(s) do
      if c.area == G.hand then hand_count = hand_count + 1
      else joker_count = joker_count + 1 end
    end
    if hand_count >= 2 then return true end
    if joker_count >= 2 then return true end
    if joker_count >= 1 and G.jokers and #G.jokers.cards >= 2 then return true end
    if hand_count == 0 and joker_count == 0 then
  ...
  use = function(self, card, area, copier)
    ...
    local hand_sel = {}
    for _,c in ipairs(s) do
      if c.area == G.hand then hand_sel[#hand_sel+1]=c
      else joker_sel[#joker_sel+1]=c end
    end

    if #hand_sel >= 2 then
      table.sort(hand_sel, function(a,b)
        local ai,bi = 999,999
      ...
      local left, right = hand_sel[1], hand_sel[2]
      G.E_MANAGER:add_event(Event({ trigger="after", delay=0.1,
        func=function()
          if right.config and right.config.card and next(right.config.card) then
            left:set_base(right.config.card)
            left:set_ability(right.config.center)
            UC.copy_extras(right, left)
          end
          card_eval_status_text(left,"extra",nil,nil,nil,{message=localize("k_copied_ex"),colour=G.C.PURPLE})
          return true
        end
      }))
    elseif #joker_sel >= 2 then
      local src, dst = joker_sel[1], joker_sel[2]
      G.E_MANAGER:add_event(Event({ trigger="after", delay=0.1,
        func=function()
    ...
    elseif #joker_sel == 1 and #G.jokers.cards >= 2 then
      local src = joker_sel[1]
      local src_idx = nil
      for i,c in ipairs(G.jokers.cards) do
        if c == src then src_idx=i; break end
      end
      if src_idx then
    ...
    elseif #joker_sel == 0 and #hand_sel == 0 then
      if G.jokers and #G.jokers.cards >= 2 then
        local src_idx = math.random(#G.jokers.cards)
        local src = G.jokers.cards[src_idx]
        local neighbor_idx = src_idx < #G.jokers.cards and src_idx+1 or src_idx-1
        local dst = G.jokers.cards[neighbor_idx]
        G.E_MANAGER:add_event(Event({ trigger="after", delay=0.1,
          func=function()
      ...
      elseif G.hand and #G.hand.cards >= 2 then
        local sel = {}
        for _,c in ipairs(G.hand.cards) do sel[#sel+1]=c end
        if #sel >= 2 then
...
SMODS.Consumable:take_ownership("immolate", {
  can_use = function(self, card)
    if UC.force_hand or UC.hand_selected() then
  ...
  use = function(self, card, area, copier)
    local hand_mode = UC.force_hand or UC.hand_selected()
    UC.force_hand = false
    G.E_MANAGER:add_event(Event({ trigger="after", delay=0.1,
      func=function()
        if hand_mode then
...
SMODS.Consumable:take_ownership("ankh", {
  ...
  use = function(self, card, area, copier)
    ...
    if hand_mode then
      local chosen = nil
      if G.hand and G.hand.highlighted then
        chosen = G.hand.highlighted[1]
      end
      if not chosen and G.hand and #G.hand.cards > 0 then
        chosen = G.hand.cards[math.random(#G.hand.cards)]
      end
      ...
      G.E_MANAGER:add_event(Event({ trigger="after", delay=0.4,
        func=function()
          local copy = copy_card(chosen, nil, nil, G.deck)
